...
static uint8_t dht_pin_index = 0;
...
*/
static void set_gpio_output_mode() {
    dht_gpio_port->MODER &= ~(3U << (dht_pin_index * 2));
    dht_gpio_port->MODER |=  (1U << (dht_pin_index * 2));
}

static void set_gpio_input_mode() {
    dht_gpio_port->MODER &= ~(3U << (dht_pin_index * 2));  // input = 00
}
...
void dht11_init(GPIO_TypeDef *gpio_port, uint16_t gpio_pin, TIM_HandleTypeDef *tim)
{
    ...
    dht_pin_index = get_pin_index(gpio_pin);
...
dht11_data dht11_data_read() {
    ...
    // Enforce minimum 1 second delay between reads
    static uint32_t last_read_time = 0;
    uint32_t now = HAL_GetTick();

    if (now - last_read_time < 1200) {   // 1.2s for extra safety
        HAL_Delay(1200 - (now - last_read_time));
    }
    ...
    while (HAL_GPIO_ReadPin(dht_gpio_port, dht_gpio_pin) == GPIO_PIN_SET) {
        if (check_timeout(1000)) { // If improper response from DHT11 timeout and throw error
        	//all timeouts changed from 200
            results.error = true;
            return results;
        }
    }
    ...
    for (uint8_t i = 0; i < 40; i++) {
        ...
        while (HAL_GPIO_ReadPin(dht_gpio_port, dht_gpio_pin) == GPIO_PIN_SET) { // Wait for DHT11 to pull low indicating next bit
            if (check_timeout(200)) {
                results.error = true;
                printf("Timeout waiting for DHT11 to pull LOW\n");
                return results;
            }
        }
    }
